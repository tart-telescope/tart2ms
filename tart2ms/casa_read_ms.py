...
def read_ms(ms_file, num_vis, angular_resolution, channel=0, field_id=0, ddid=0, snapshot=0, pol=0):
    ...
    logger.info(f"colnames{ms.colnames()}")
    logger.info(f"keywordnames{ms.keywordnames()}")
    logger.info(f"fields{ms.fieldnames()}")
    ...
    fields = table(subt.getkeyword("FIELD"))
    ...
    logger.info(f"Field {name} (index {field_id}): Phase Dir {np.degrees(phase_dir)}, t={field_time}")

    times = subt.getcol("TIME")
    logger.info(f"times {times.shape}")
    time_steps, inverse = np.unique(times, return_inverse=True)
    logger.info(f"time_steps {time_steps - time_steps[0]}")
    logger.info(f"inverse {inverse}")

    t_snapshot = time_steps[snapshot]

    snapshot_indices = np.where((inverse == snapshot))[0]
    logger.info(f"Snapshot {t_snapshot}")
    # logger.info(f"snapshot_indices {snapshot_indices}")
    logger.info(f"snapshot_indices {snapshot_indices.shape}")

    uvw = subt.getcol("UVW")
    logger.info("uvw {}".format(uvw.shape))

    ant1 =  subt.getcol("ANTENNA1")
    ant2 =  subt.getcol("ANTENNA2")
    logger.info(f"ant = {ant1.shape}")


    flags = subt.getcol("FLAG")
    logger.info(f"flags = {flags.shape}")
    logger.info(f"channel = {channel.shape}")
    ...
    logger.info(spw.colnames())

    frequencies = spw.getcol("CHAN_FREQ")[0]
    
    frequency = frequencies[channel]
    logger.info(f"Frequencies = {frequencies.shape}")
    ...
    #flags = flags[snapshot_indices,:,:]
    logger.info("Snapshot Flags: {}".format(flags.shape))
    ...
    #
    ant1 =  ant1[baseline_indices]
    ant2 =  ant2[baseline_indices]
    weight_spectrum =  weight_spectrum[baseline_indices]

    raw_vis = raw_vis[baseline_indices]
    logger.info(f"Raw Vis {raw_vis.shape}")


    u_arr = uvw[baseline_indices,0]
    v_arr = uvw[baseline_indices,1]
    w_arr = uvw[baseline_indices,2]
    logger.info(f"u_arr {u_arr.shape}")

    rms_arr = np.sqrt(1.0 / weight_spectrum)

    logger.info(f"rms_arr {rms_arr.shape}")

    time = times[baseline_indices]
    logger.info(f"time {time.shape}, timestamp {times[0]}")

    seconds = (time - time[0])

    epoch_seconds = np.mean(times)
    logger.info(f"time {epoch_seconds}, timestamp {times[0]}")
...
if __name__=="__main__":
    ...
    #parser.add_argument('--channels', type=list, default=[0], help="First frequency channel.")
    #parser.add_argument('--channels', action='append', help='<Required> Set flag', required=True)
    ...
    sdev = plt.Circle((np.mean(np.real(corrected_vis)), np.mean(np.imag(corrected_vis))), np.std(np.abs(corrected_vis)), fill=False)
    axes.set_aspect( 1 )
