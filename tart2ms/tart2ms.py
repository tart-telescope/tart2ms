...
# (e.g. during pickle/serialization). Patch in a safer version.
_orig_getattr = Dataset.__getattr__
...
def ms_create(
    ...
    phase_altaz = SkyCoord(
        alt=[90.0 * u.deg] * len(obstime),
        az=[0.0 * u.deg] * len(obstime),
        obstime=obstime,
        frame="altaz",
        location=location,
    )
    ...
    ant_lon_lat = [
        ac.offset_by(lon=lon * u.deg, lat=lat * u.deg, posang=theta, distance=d)
        for theta, d in zip(ant_posang, ant_distance)
    ]
    ant_locations = [
        EarthLocation.from_geodetic(
            lon=lon, lat=lat, height=loc["alt"] * u.m, ellipsoid="WGS84"
        )
    ...
    if phase_center_policy == "instantaneous-zenith":
        ...
        direction = np.deg2rad(
            np.array(
                [
                    [
                        phase_center_policy.icrs.ra.value,
                        phase_center_policy.icrs.dec.value,
                    ]
                ]
            ).reshape(1, 2, 1)
        )
        ...
        direction = direction[:, :, direction.shape[2] // 2].reshape(
            1, 2, 1
        ...
        if len(fsrc) > 0:
            ...
            if fsrc[0]["position"]["FRAME"] == "Special Body":
                ...
                LOGGER.critical(
                    f"User caution: enabling non-sidereal tracking of '{fn}'. Indicated field centre will be "
                ...
                direction = np.deg2rad(
                    np.array([[body.icrs.ra.value, body.icrs.dec.value]]).reshape(
                        1, 2, 1
                    )
                )
            else:
                ...
                direction = np.deg2rad(
                    np.array([[crd.icrs.ra.value, crd.icrs.dec.value]]).reshape(1, 2, 1)
                )
    else:
        raise ValueError(
            f"phase_center_policy must be one of "
    ...
    if use_special_fn:
        field_name = da.asarray(
            np.array([use_special_fn] * direction.shape[2]),
            dtype=object,
    ...
    field_direction = da.asarray(
        direction.T.reshape(direction.shape[2], 1, 2).copy(),
        chunks=(direction.shape[2], None, None),
    )  # nrow x npoly x 2
    ...
    dataset = Dataset(
        {
            "TELESCOPE_NAME": (
                ("row",),
                da.asarray(
                    np.asarray([override_telescope_name], dtype=object), chunks=1
                ),
            ),
            "OBSERVER": (
                ("row",),
                da.asarray(np.asarray(["Tim"], dtype=object), chunks=1),
            ),
            "TIME_RANGE": (
                ("row", "obs-exts"),
                da.asarray(np.array([[epoch_s[0], epoch_s[-1]]]), chunks=1),
            ),
        }
    )
    ...
    for corr_type in corr_types:
        ...
        dask_corr_type = da.from_array(corr_type, chunks=len(corr_type))[None, :]
    ...
    spw_ids, pol_ids = zip(
        *product(range(len(num_freq_channels)), range(len(corr_types)))
    )
    ...
    if np.array(timestamps).size > 1 and uvw_generator != "casacore":
        LOGGER.warning(
            f"You should not use '{uvw_generator}' mode to generate UVW coordinates"
            f"for multi-timestamp databases. Your UVW coordinates will be wrong"
        )
    if (
        uvw_generator == "telescope_snapshot"
        and phase_center_policy == "instantaneous-zenith"
    ):
    ...
    for ddid, (spw_id, pol_id) in enumerate(zip(spw_ids, pol_ids)):
        ...
        if phase_center_policy == "instantaneous-zenith":
            ...
            scan = (
                np.arange(len(epoch_s), dtype=int).repeat(nbl) + 1
            )  # offset to start at 1, per convention
        ...
        mean_sidereal_day = 23 + 56 / 60.0 + 4.0905 / 3600.0  # hrs
        # degrees per second at equator
        sidereal_rate = 360.0 / (mean_sidereal_day * 3600)
        ...
        if uvw_generator == "telescope_snapshot":
            ...
            ):  # rephase or non-rephase single field database
                raise RuntimeError(
                    "Telescope snapshot UVW mode may only be used for zenethal snapshotting mode"
                )
            bl_pos = np.array(ant_pos)[baselines]
            uu_a, vv_a, ww_a = -(bl_pos[:, 1] - bl_pos[:, 0]).T
            # Use the - sign to get the same orientation as our tart projections.
            ...
            if (
                ...
                if (
                    ...
                    centroid_direction = zenith_directions[
                        zenith_directions.shape[0] // 2, :
                    ].reshape(1, 2)
                elif isinstance(phase_center_policy, SkyCoord):
                    centroid_direction = np.deg2rad(
                        np.array(
                            [
                                [
                                    phase_center_policy.icrs.ra.value,
                    ...
                    fn = phase_center_policy.replace("rephase-", "")
                    # 3CRR currently only catalog with special names
                    catalog_positions = catalog_reader.catalog_factory.from_3CRR(
                        fluxlim15=0.0
                    )
                    named_positions = read_known_phasings()
                    fsrc = list(filter(lambda x: x.name == fn, catalog_positions))
                    if len(fsrc) > 0:
                        centroid_direction = np.array(
                            [[fsrc[0].rarad, fsrc[0].decrad]]
                        ).reshape(1, 2)
                    else:  # otherwise it is in the named positions list
                        fsrc = list(
                            filter(lambda x: x["name"].upper() == fn, named_positions)
                        )
                        if len(fsrc) == 0:
                            raise RuntimeError(f"Unknown named source {fn}")
                        if fsrc[0]["position"]["FRAME"] == "Special Body":
                            centroid_direction = np.empty(
                                (len(obstime), 2), dtype=float
                            )
                            for ti, tt in enumerate(obstime):
                                body = ac.get_body(fn, tt, location=location)
                                centroid_direction[ti, :] = np.deg2rad(
                                    np.array([body.icrs.ra.value, body.icrs.dec.value])
                                )
                        else:
                            # we assume we can parse it with astropy
                            ra = fsrc[0]["position"]["RA"]
                            ...
                            centroid_direction = np.deg2rad(
                                np.array(
                                    [[crd.icrs.ra.value, crd.icrs.dec.value]]
                                ).reshape(1, 2)
                            )
        ...
        if (
            phase_center_policy == "no-rephase-obs-midpoint"
            and obs_length > snapshot_length_cutoff
        ):
            LOGGER.critical(
                f"You are choosing to set the field phase direction at the centre point "
        ...
        if fill_model:
            ...
            if write_extragalactic_catalogs:
                LOGGER.info(
                    f"Predicting celestial catalog positions for {len(epoch_s)} timestamps"
                )
                celestial_model = predict_model(
                    dask_data.shape,
                    dask_data.chunks,
                    dask_data.dtype,
                    uvw_data,
                ...
                LOGGER.info(
                    f"Predicting Sun and Moon positions for {len(epoch_s)} timestamps"
                )
                cat_sources = get_solar_system_bodies(obstime, location)
                solar_model = predict_model(
                    dask_data.shape,
                    dask_data.chunks,
                    dask_data.dtype,
                    uvw_data,
        ...
        if (
            ...
            if centroid_direction.shape[0] == 1:
                new_phase_dir = SkyCoord(
                    centroid_direction[0, 0] * u.rad,
                    centroid_direction[0, 1] * u.rad,
                    frame="icrs",
                )
                new_phase_dir_repr = (
                    f"{new_phase_dir.ra.hms[0]:02.0f}h{new_phase_dir.ra.hms[1]:02.0f}m{new_phase_dir.ra.hms[2]:05.2f}s "
    ...
    LOGGER.info(
        f"Measurement set writing complete. Took {elapsed // 60:.0f}m{elapsed % 60.0:.2f}s"
    )
...
def __load_ext_ant_pos(fn, ack=True):
    ...
    if not os.path.exists(fn) and not os.path.isfile(fn):
        raise RuntimeError(
            f"External antenna position file '{fn}' is not a valid file or does not exist"
        )
    with open(fn) as ffn:
        ant_pos_dict = json.loads(ffn.read())
        if "antenna_positions" not in ant_pos_dict.keys():
            raise RuntimeError(
                "Expected a dictionary keyed on 'antenna_positions'. Key does not exist"
            )
        ant_pos = ant_pos_dict["antenna_positions"]
        if not isinstance(ant_pos, list):
            raise RuntimeError(
                "Expected a list of ENU coordinates in external antenna positions file"
            )
        if not all(
            map(
                lambda x: (
                    isinstance(x, list)
                    and len(x) == 3
                    and all(map(lambda xn: isinstance(xn, float), x))
                ),
                ant_pos,
            )
        ):
    ...
    if ack:
        LOGGER.warning(
            f"Per user request will override antenna positions from externally provided database {fn}"
        )
    return ant_pos
...
def ms_from_hdf5(
    ...
    for ih5, h5 in enumerate(h5file):
        with h5py.File(h5, "r") as h5f:
            ...
            if not np.isclose(ant_pos_orig, ant_pos, atol=1.0e-1, rtol=1.0).all():
                raise RuntimeError(
                    "The databases you are trying to concatenate have different antenna layouts. "
            ...
            for check_key in [
                ...
                if isinstance(orig_dico_info[check_key], float):
                    config_same = config_same and np.isclose(
                        orig_dico_info[check_key], config_json[check_key]
                    )
                elif isinstance(orig_dico_info[check_key], list):
                    config_same = config_same and np.all(
                        np.array(orig_dico_info[check_key])
                        == np.array(config_json[check_key])
                    )
                else:
                    config_same = (
                        config_same
                        and orig_dico_info[check_key] == config_json[check_key]
                    )

            if not config_same:
                raise RuntimeError(
                    "The databases you are trying to concatenate have different configurations. "
                    "This is not yet supported. You could try running CASA virtualconcat to "
                    "concatenate such heterogeneous databases"
                )
            gains = h5f["gains"][:]
            ...
            for ts, v in zip(timestamps, hdf_vis):
                if filter_start_utc and ts < filter_start_utc:
                    continue
                if filter_end_utc and ts > filter_end_utc:
                    continue
                tscount += 1
                ts_this_h5 += 1
            ...
            if fetch_sources:
                ...
                if lat is None or lon is None:
                    raise RuntimeError(
                        "Telescope location is unavailable from the provided databases"
                    )
                ...
                all_sources += (
                    ...
                    else [None] * len(online_sources_timestamps)
                )
    ...
    if tscount == 0:
        raise RuntimeError(
            "Time filtering criteria resulted in an empty database. Goodbye!"
        )
...
def ms_from_json(
    ...
    for ijdi, jdi in enumerate(json_data):
        ...
        for check_key in [
            ...
            if check_key not in info["info"] or check_key not in orig_dico_info:
                if check_key not in opt_keys:
                    ...
                    LOGGER.critical(
                        f"Key {check_key} missing from database, but appears to be optional."
                        f"You may be using old databases!"
                    )
                    continue
            if isinstance(orig_dico_info[check_key], float):
                config_same = config_same and np.isclose(
                    orig_dico_info[check_key], info["info"][check_key]
                )
            elif isinstance(orig_dico_info[check_key], list):
                config_same = config_same and np.all(
                    np.array(orig_dico_info[check_key])
                    == np.array(info["info"][check_key])
                )
            else:
                config_same = (
                    config_same and orig_dico_info[check_key] == info["info"][check_key]
                )
        ...
        for d in jdi["data"]:
            ...
            if filter_start_utc and timestamp < filter_start_utc.replace(
                tzinfo=timestamp.tzinfo
            ):
                continue
            if filter_end_utc and timestamp > filter_end_utc.replace(
                tzinfo=timestamp.tzinfo
            ):
                continue
            tscount += 1
            if fetch_sources:
                ...
                online_list, online_timestamps = __fetch_sources(
                ...
                online_timestamps = (
                    [timestamp] if not online_list else online_timestamps
                )
            else:
